...
export const useSalarySettings = () => {
  ...
  const getSalaryForDate = React.useCallback((date: Date): MonthlySalary => {
    ...
    if (salaryHistory) {
      const historyKeys = Object.keys(salaryHistory).sort().reverse();
  ...
  const getShiftSettingsForDate = React.useCallback((date: Date) => {
    ...
    const history = [...(settings.shiftHistory || [])].sort((a, b) => b.startDate.localeCompare(a.startDate));
    
    const entry = history.find(h => h.startDate <= dateStr);
